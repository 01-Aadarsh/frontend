...
# Which documents are India-domestic law vs. international instruments —
# drives the jurisdiction toggle's retrieval filter (see retrieval/*.py).
# Everything not listed here defaults to "national". Only add a filename
# here when it's genuinely an international treaty/convention text, not an
# Indian Act that happens to implement one (e.g. the Biological Diversity
# Act is India's domestic ABS law, even though it implements the CBD/Nagoya
# obligations internationally — it stays "national").
INTERNATIONAL_DOCUMENTS = {
    "WIPO_GRATK_Treaty_2024.pdf",
}


def jurisdiction_for(source_file: str) -> str:
    return "international" if source_file in INTERNATIONAL_DOCUMENTS else "national"
...
def store_in_pgvector(conn: psycopg.Connection, chunks: list[Chunk], embeddings) -> None:
    ...
    rows = [
        (
            ...
            chunk.text,
            jurisdiction_for(chunk.source_file),
            embedding,
        )
        ...
        pickle.dump(
            {
                ...
                # Parallel array, same order/index as chunk_ids — lets
                # bm25_search.py filter by jurisdiction without a DB round
                # trip per query.
                "jurisdictions": [jurisdiction_for(chunk.source_file) for chunk in chunks],
            },
            handle,
        )
