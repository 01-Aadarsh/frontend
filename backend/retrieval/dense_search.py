...
def search(query: str, top_k: int = 20, jurisdiction: str | None = None) -> list[dict]:
    ...
    `jurisdiction`, if given ("national" | "international"), restricts the
    search to chunks from documents tagged with that jurisdiction (see
    ingestion/indexer.py's INTERNATIONAL_DOCUMENTS) -- filtered in SQL before
    the nearest-neighbor ordering, not after, so top_k is actually top_k of
    the requested jurisdiction.
    """
    model = _get_model()
    embedding = model.encode(query, normalize_embeddings=True)

    where_clause = "WHERE jurisdiction = %s" if jurisdiction else ""
    params = (embedding,) + ((jurisdiction,) if jurisdiction else ()) + (embedding, top_k)
