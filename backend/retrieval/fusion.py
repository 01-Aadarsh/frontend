...
def search(query: str, top_k: int = 20, jurisdiction: str | None = None) -> list[dict]:
    ...
    `jurisdiction` ("national" | "international" | None) passes straight
    through to both retrievers -- see their docstrings.
    """
    bm25_results = bm25_search(query, top_k=top_k, jurisdiction=jurisdiction)
    dense_results = dense_search(query, top_k=top_k, jurisdiction=jurisdiction)
