...
`jurisdiction` is optional — `"national"` or `"international"`, filters
retrieval to only documents tagged with that jurisdiction (see `ingestion/
indexer.py`'s `INTERNATIONAL_DOCUMENTS` — today only the WIPO GRATK Treaty is
tagged `international`, everything else is `national`, including India's
domestic ABS law even though it implements international CBD/Nagoya
obligations). Omit or `null` for no filter. **The frontend's jurisdiction
toggle should always send one of the two strings** — that's what keeps
national and international answer-sets from ever conflating, per the PS.

`category` is optional — free text from the formulation-classification step
(e.g. `"classical medicine"`, `"new drug"`, `"phytopharmaceutical"`,
`"proprietary medicine"`, `"Ayurveda-Aahar"`, `"cosmetic"`). Prepended as a
short context clause ahead of the question before retrieval and generation —
it isn't a strict enum server-side, whatever string the frontend sends is
what gets prepended verbatim (`"For a {category} formulation: {question}"`).
Omit or `null` if the user skipped classification.
...
### `GET /sources/<source_file>`

Static file serving of the actual indexed PDFs (mounted from `DATA_DIR`, same
folder `ingestion/indexer.py` reads from) — not a JSON endpoint. Use a
citation's `source_file` verbatim as the path segment, e.g.
`GET /sources/Patents_Act_1970.pdf`. Append `#page=N` (using the citation's
`page_number`) when linking to it from an `<a>` tag — most browsers' native
PDF viewer jumps to that page; this is client-side PDF-viewer behavior, not
something the server does.

404s if `DATA_DIR` doesn't exist on the running server (logged as a startup
warning, not a hard failure — `/query` and citations work either way, this
only affects a "view source PDF" link if you build one).
...
| `text` | string | The exact retrieved chunk text — the same text the LLM was given as context, not a re-generated summary or excerpt. Safe to render verbatim in an expandable "view exact snippet" UI. Can be a few hundred to ~2000 characters (chunking targets ~500 tokens with ~50-token overlap). | No — always the real stored chunk text. |
| `confidence` | integer | 0-100, a sigmoid-squashed transform of the cross-encoder's rerank score for this chunk. **Not a calibrated probability of correctness** — it's a readable version of the same score already used to select this chunk as relevant. Fine to render as a confidence bar/badge/percentage; don't present it to users as a rigorous accuracy figure. | No. |
